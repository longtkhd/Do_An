...
const BoothOwnerList: React.FC = observer(() => {
  const initialParams = {
    sort: 'id',
    order: 'descend',
  ...
  const handleDelete = (id?: number) => {
    Modal.confirm({
      ...
      content: t('user.DO_YOU_WANT_TO_DELETE_THIS_BOOTH_OWNER'),
      ...
      onOk: async () => {
        ...
        if (res && !userStore.error) {
          ...
          message.success(t('user.DELETE_BOOTH_OWNER_SUCCESSFULLY'));
        }
      },
    });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'booth',
      sorter: true,
      render: (text: string, item: IUser) => item?.booth?.name,
    },
    {
    ...
    },
    {
      ...
      render: (text: any, item: IUser) => {
        return (
          <Space size={5}>
            <Link to={`/cms/users/booth-owners/edit/${item.id}`}>
              <IconBtn>
                <EditOutlined />
              </IconBtn>
            </Link>
  ...
  return (
    <Card
      title={<StyledHeading>{t('user.BOOTH_OWNER_LISTING')}</StyledHeading>}
      bordered={false}
      extra={
        <Button type="primary">
          <Link to="/cms/users/booth-owners/create">
