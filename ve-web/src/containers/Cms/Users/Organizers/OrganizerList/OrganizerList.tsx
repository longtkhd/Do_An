...
const OrganizerList: React.FC = observer(() => {
  const initialParams = {
    sort: 'id',
    order: 'descend',
  ...
  const handleDelete = (id?: number) => {
    Modal.confirm({
      ...
      content: t('user.DO_YOU_WANT_TO_DELETE_THIS_ORGANIZER'),
      ...
      onOk: async () => {
        ...
        if (res && !userStore.error) {
          ...
          message.success(t('user.DELETE_ORGANIZER_SUCCESSFULLY'));
        }
      },
    });
  };
  ...
  const columns = [
    ...
    {
      title: t('user.ORGANIZER'),
      dataIndex: 'organizer',
      sorter: true,
      render: (text: string, item: IUser) => item?.organizer?.name,
    },
    {
    ...
    },
    {
      ...
      render: (text: any, item: IUser) => {
        return (
          <Space size={5}>
            <Link to={`/cms/users/organizers/edit/${item.id}`}>
              <IconBtn>
                <EditOutlined />
              </IconBtn>
            </Link>
  ...
  return (
    <Card
      title={<StyledHeading>{t('user.ORGANIZER_LISTING')}</StyledHeading>}
      bordered={false}
      extra={
        <Button type="primary">
          <Link to="/cms/users/organizers/create">
